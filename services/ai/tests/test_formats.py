...
def test_image_formats_need_no_local_extraction() -> None:
    ...
    # PSD bytes are extracted locally; only derived evidence reaches a provider.
    assert adapter_for("psd").prepare is not None


def test_unknown_format_has_no_adapter() -> None:
    assert adapter_for("svg") is None
...
def test_prompt_selection_is_driven_by_the_adapter_prefix() -> None:
    ...
    _, psd_user = load_annotation_prompt(
        "campaign.psd", is_first_version=False, operation_prefix="PSD "
    )
    assert image_user != psd_user
    assert "logo.png" in image_user
    assert "campaign.psd" in psd_user
...
async def test_annotate_rejects_a_format_without_an_adapter() -> None:
    ...
    object.__setattr__(request, "format", "svg")

    with pytest.raises(UnsupportedFormatError, match="cannot annotate 'svg'"):

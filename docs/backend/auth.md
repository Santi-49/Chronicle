...
Chronicle uses a Google OAuth client of type **Desktop app**. Electron first requires a successful,
short-timeout `GET /health` preflight, then opens the operating system's default external browser
(not an Electron `BrowserWindow`/webview), binds a temporary random `127.0.0.1` callback, and uses
state + nonce + PKCE S256. It exchanges the

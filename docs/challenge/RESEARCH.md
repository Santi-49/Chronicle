...
- 2026-07-22 — GOOGLE EXISTING-ACCOUNT LINK DECISION (POST-03; supersedes the narrower first-admin bootstrap rule): a verified Google email matching any existing active Chronicle account attaches the stable Google `sub` on first login, including password accounts and `FIRST_ADMIN_EMAIL`. Later logins resolve by `sub`, inactive accounts remain blocked, and email matching never replaces a different linked Google identity — team direction + implementation review
- 2026-07-23 — PACKAGED GOOGLE TOKEN-EXCHANGE FINDING (POST-03): release 0.5.0 successfully completed system-browser authorization and loopback callback but Google's token endpoint returned `400 invalid_request`; the release embedded the Desktop client ID while omitting the matching client secret that local `.env` supplied. Google calls `client_secret` optional in its installed-app OAuth guide but required in its OpenID Connect token reference, and notes installed-app client values cannot be confidential. Chronicle now injects the matching value from a GitHub Actions secret, retains PKCE as the security boundary, and rejects release packaging when the value is absent — live release diagnosis + [Google desktop OAuth](https://developers.google.com/identity/protocols/oauth2/native-app) + [Google OpenID Connect reference](https://developers.google.com/identity/openid-connect/reference)
- 2026-07-23 — RELEASE BACK-SYNC DECISION (MVP-12): after Release Please creates a durable release, GitHub Actions automatically merges released `main` into the latest `dev`. The job is serialized, fetches both branch tips immediately before merging, never force-pushes, and fails without changing the remote on conflicts or concurrent updates. It uses the built-in `GITHUB_TOKEN`, whose pushes do not recursively trigger workflows, preventing release-loop automation while keeping version metadata and squash-merge ancestry current — team workflow review + [GitHub Actions token behavior](https://docs.github.com/en/actions/concepts/security/github_token) + [actions/checkout authenticated push](https://github.com/actions/checkout)

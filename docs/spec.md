...
trade-off: development needs Python 3.12, while native Windows/macOS installers bundle a
...
| 8 | **AI sidecar health/provider packaging** — development needs Python 3.12+, and an installed sidecar can fail to start or lack a selected provider integration | AI demo beat fails; teammates stuck on setup | Windows and macOS installers bundle a self-contained Gemini/OpenAI/Anthropic sidecar and smoke-check every frozen provider import plus `/health`; app still degrades gracefully (versions capture, jobs queue, UI shows *pending*) |
...
- **Is the desktop app easy to demo/ship?** Demo: yes — `npm run dev` runs the full app. Packaging produces unsigned, self-contained Windows NSIS and macOS DMG installers with the Gemini/OpenAI/Anthropic sidecar. SmartScreen or Gatekeeper may warn; signing, notarization, and in-app auto-update remain post-MVP.

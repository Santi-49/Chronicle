...
| 8 | **AI sidecar health/provider packaging** — development needs Python 3.12+, and an installed sidecar can fail to start or lack a selected provider integration | AI demo beat fails; teammates stuck on setup | The Windows installer bundles a self-contained Gemini/OpenAI/Anthropic sidecar and smoke-checks every frozen provider import plus `/health`; app still degrades gracefully (versions capture, jobs queue, UI shows *pending*) |
...
- **Is the desktop app easy to demo/ship?** Demo: yes — `npm run dev` runs the full app. MVP-12 produces an unsigned, self-contained Windows NSIS installer with the Gemini/OpenAI/Anthropic sidecar; SmartScreen may warn. macOS packaging/signing and in-app auto-update remain post-MVP.

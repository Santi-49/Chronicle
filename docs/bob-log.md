...
| 2026-07-19 | Santi R | *Pending — implemented with Claude Code; run IBM Bob review before submission.* Added token usage + estimated cost to the C3 annotate/embed responses, moved default provider/generic key/per-task models + per-task pricing into `.env.example` (env fallback in the service), removed hardcoded Gemini from code/tests, renamed the smoke script provider-neutral (prints tokens + cost), and added `make smoke-ai`; 43 Python + full desktop suite green | MVP-09 — tokens/cost + env config (`feat/mvp-09-python-ai`) |

...
| **Startup & background** | Three ordered choices. **Keep capturing after I close the window** (default on) turns a window close into a hide, leaving Chronicle resident in the notification area with capture, the AI queue, and search intact; turning it off restores quit-on-close. **Start Chronicle when I sign in** registers a login item, and the nested **Open the Chronicle window at sign-in** decides between restoring the UI and resuming silently in the tray. The startup preference is read back from the operating system on every visit rather than stored in C5, because it can be revoked from Task Manager or System Settings; it is disabled with an explanation in development builds and on platforms without a login-item API. With background capture off, the window option is forced on — otherwise a login launch would have neither a window nor a tray icon. |

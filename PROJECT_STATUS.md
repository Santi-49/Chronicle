...
| AI summaries | Merged (MVP-09); packaged in MVP-12; POST-01 complete; POST-02 PSD in progress | The Python AI service lives in `services/ai/` (package `chronicle_ai`, FastAPI + LangChain); Electron owns its queue/client/lifecycle. Windows x64 and macOS Apple Silicon packaging include a self-contained, health-smoked Gemini/OpenAI/Anthropic sidecar and prompt, so installed users need no Python. POST-02 now has a bounded local PSD extractor and a token-efficient structure/preview path. The 61 Python tests, 141 desktop tests, production build, and packaged PSD-route smoke test pass. PSD stays unchecked because the controlled Gemini run was blocked by the locally configured credential returning HTTP 401 before inference; PSB is not part of this increment. |

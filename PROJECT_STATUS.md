...
| AI summaries | Merged (MVP-09); packaged in MVP-12 | The Python AI service lives in `services/ai/` (package `chronicle_ai`, FastAPI + LangChain); Electron owns its queue/client/lifecycle. The Windows installer includes a self-contained, health-smoked Gemini/OpenAI/Anthropic sidecar and prompt, so installed users need no Python. Bedrock is excluded because its multi-field AWS credentials do not fit the current per-provider single-key contract. |
...
| `main` automation/protection | Team lead | Before merging MVP-12 | Configure `RELEASE_PLEASE_TOKEN`, allow Action-created PRs, and require the three **Main PR CI** jobs only for PRs targeting `main`; keep direct pushes disabled. Release Please PRs reuse the Desktop check for a lightweight version guard and skip the two implementation jobs successfully. `dev` has no required CI by team decision. |

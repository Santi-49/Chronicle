...
```bash
git clone https://github.com/Santi-49/Chronicle.git
cd Chronicle
make setup
make run
```
...
### Enable AI summaries and semantic search

```bash
make setup-ai
```

Open **Settings > AI**, save a provider API key, and select the models for change summaries and
embeddings. Keys are encrypted through Electron `safeStorage`. In the default BYOK path, the local
sidecar sends the required task inputs directly to the provider you selected. Chronicle versions
continue to capture when AI is disabled, unavailable, or offline.
...
Chronicle's creative version library remains local. AI is optional and uses the provider configured
by the user. Account-backed settings and encrypted key backup are separate from the core desktop
workflow. The precise implemented and planned data boundaries are documented here:
